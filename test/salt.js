...
async function main() {
  ...
  ok('well-known host door', /\/host/.test(wk.host) && wk.packs && wk.packs.length >= 3);

  let threw = false;
  try { await salt.create({}); } catch (e) { threw = e.status === 400; }
  ok('create requires to or amount', threw);

  const ch = await salt.create({ who: 'cursor', action: 'pay', to: 'Railway', amount: '12', unit: 'USD' });
  ok('create pending', ch.status === 'pending' && ch.id && ch.tap.indexOf(ch.id) !== -1);
  ok('demo is not live', ch.live === false);
  ok('poll matches', salt.get(ch.id).status === 'pending');

  const no = await salt.decide(ch.id, false);
  ok('no has no receipt', no.status === 'no' && !no.receipt);

  const ch2 = await salt.create({ who: 'slwurld', do: 'send', to: 'Sammy', amount: '36', unit: 'MCFL' });
  const yes = await salt.decide(ch2.id, true);
  ok('yes has receipt', yes.status === 'yes' && yes.receipt && yes.receipt.sig && yes.receipt.ok === true);
  ok('demo receipt not live', yes.receipt.live === false);

  const v = salt.verifyReceipt(yes.receipt);
  ok('verify good receipt', v.ok === true && v.receipt.id === ch2.id);
  ok('verify demo is not live', v.live === false);
  ...
  const httpWk = await salt.handleHttp('GET', '/api/salt', null);
  ok('handleHttp well-known', httpWk && httpWk.status === 200 && httpWk.json.name === 'Salt');
  const http403 = await salt.handleHttp('GET', '/.well-known/human-receipt', null);
  ok('handleHttp well-known alias', http403 && http403.json.human_required === 403);

  const httpCh = await salt.handleHttp('POST', '/api/salt/challenge', { who: 'x', action: 'ring', to: 'erik' });
  ok('handleHttp create', httpCh.status === 201 && httpCh.json.human_required === 403);
  const id = httpCh.json.id;
  ok('handleHttp poll', (await salt.handleHttp('GET', '/api/salt/challenge/' + id, null)).json.status === 'pending');
  const httpYes = await salt.handleHttp('POST', '/api/salt/challenge/' + id + '/yes', {});
  ok('handleHttp yes', httpYes.json.status === 'yes' && httpYes.json.receipt.sig);
  const ver = await salt.handleHttp('POST', '/api/salt/verify', { receipt: httpYes.json.receipt });
